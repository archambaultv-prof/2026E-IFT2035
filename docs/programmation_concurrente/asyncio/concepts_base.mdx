...
La programmation asynchrone basée sur des coroutines et l'`await` est un
paradigme présent dans de nombreux langages modernes : JavaScript
(`async`/`await`), Rust, C#, Kotlin, Swift, etc. L'idée centrale — permettre
à une unité d'exécution de se suspendre volontairement en attendant une
opération lente, sans bloquer le reste du programme — est la même quel que
soit le langage.

En Python, `asyncio` est la bibliothèque standard qui met en œuvre ce
paradigme. Elle permet d'écrire du **code concurrent** principalement pour
des tâches **I/O bound** (réseau, fichiers, bases de données, APIs).
Contrairement au multithreading classique :
...
Pour du **CPU intensif**, `asyncio` n'apporte pas de parallélisme réel.
