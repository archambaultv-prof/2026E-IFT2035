...
Ce concept est universel et présent dans pratiquement tous les langages
systèmes et de haut niveau : pthreads en C, `java.lang.Thread` en Java,
`std::thread` en C++, le module `concurrent.futures` en Python, etc. Dans ce
cours, nous utilisons le module `threading` de Python pour illustrer ces
concepts. Ce module abstrait la complexité des threads système et fournit des
outils pour la synchronisation et la communication entre threads.

### Le GIL et ses implications en Python

Contrairement à des langages comme C ou Java où les threads peuvent exploiter
pleinement le parallélisme CPU, Python utilise le **GIL (Global Interpreter
Lock)**, un mécanisme spécifique à CPython qui empêche l'exécution simultanée
de bytecode Python par plusieurs threads. Cela peut sembler contradictoire avec
l'idée du multithreading, mais le threading reste très utile dans plusieurs cas :
